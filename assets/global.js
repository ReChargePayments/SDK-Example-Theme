  ...
  renderProductInfo() {
    ...
    fetch(`${this.dataset.url}?variant=${requestedVariantId}&section_id=${this.dataset.originalSection ? this.dataset.originalSection : this.dataset.section}`)
      ...
      .then((responseText) => {
        ...
        // if the recharge-price-modifier class exists, we don't want to replace the price
        if (source && destination && !document.querySelector('.price').classList.contains('recharge-price-modifier')) destination.innerHTML = source.innerHTML;
